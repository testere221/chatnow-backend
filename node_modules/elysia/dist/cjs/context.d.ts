...
import type { RouteSchema, Prettify, ResolvePath, SingletonBase, HTTPHeaders, InputSchema } from './types';
...
export type ErrorContext<in out Route extends RouteSchema = {}, in out Singleton extends SingletonBase = {
    ...
    cookie: undefined extends Route['cookie'] ? Record<string, Cookie<string | undefined>> : Record<string, Cookie<string | undefined>> & {
    ...
    status: {} extends Route['response'] ? typeof status : <const Code extends keyof Route['response'] | InvertedStatusMap[Extract<InvertedStatusMapKey, keyof Route['response']>], T extends Code extends keyof Route['response'] ? Route['response'][Code] : Code extends keyof StatusMap ? Route['response'][StatusMap[Code]] : never>(code: Code, response: T) => ElysiaCustomStatusResponse<Code, T>;
    /**
...
export type Context<in out Route extends RouteSchema = {}, in out Singleton extends SingletonBase = {
    ...
    query: undefined extends Route['query'] ? {} extends NonNullable<Singleton['resolve']['query']> ? Record<string, string> : Singleton['resolve']['query'] : PrettifyIfObject<Route['query'] & Singleton['resolve']['query']>;
    params: undefined extends Route['params'] ? undefined extends Path ? {} extends NonNullable<Singleton['resolve']['params']> ? Record<string, string> : Singleton['resolve']['params'] : Path extends `${string}/${':' | '*'}${string}` ? ResolvePath<Path> : never : PrettifyIfObject<Route['params'] & Singleton['resolve']['params']>;
    headers: undefined extends Route['headers'] ? {} extends NonNullable<Singleton['resolve']['query']> ? Record<string, string | undefined> : Singleton['resolve']['headers'] : PrettifyIfObject<Route['headers'] & Singleton['resolve']['headers']>;
    cookie: undefined extends Route['cookie'] ? Record<string, Cookie<unknown>> : Record<string, Cookie<unknown>> & Prettify<{
        [key in keyof Route['cookie']]-?: Cookie<Route['cookie'][key]>;
    } & {
        [key in keyof Singleton['resolve']['cookie']]-?: Cookie<Singleton['resolve']['cookie'][key]>;
    }>;
    server: Server | null;
    redirect: Redirect;
    set: {
