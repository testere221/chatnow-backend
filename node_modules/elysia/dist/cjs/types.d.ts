...
export type ResolvePath<Path extends string> = Path extends PathParameterLike ? _ResolvePath<Path> : {};
...
export type UnwrapSchema<Schema extends AnySchema | string | undefined, Definitions extends DefinitionBase['typebox'] = {}> = undefined extends Schema ? unknown : Schema extends TSchema ? Schema extends OptionalField ? Partial<TImport<Definitions & {
...
type ExtractOnlyResponseFromMacro<A> = IsNever<A> extends true ? {} : {} extends A ? {} : Extract<A, AnyElysiaCustomStatusResponse> extends infer A ? IsNever<A> extends true ? {} : {
    return: A extends ElysiaCustomStatusResponse<any, infer Value, infer Status> ? {
...
export type MacroToContext<in out MacroFn extends Macro = {}, in out SelectedMacro extends BaseMacro = {}, in out Definitions extends DefinitionBase['typebox'] = {}, in out R extends 1[] = [1]> = Prettify<{} extends SelectedMacro ? {} : R['length'] extends 16 ? {} : UnionToIntersect<{
...
export type InlineHandler<Route extends RouteSchema = {}, Singleton extends SingletonBase = {
    ...
    200: any;
} ? Route['response'][200] | ElysiaCustomStatusResponse<200, Route['response'][200], 200> : unknown) | Route['response'][keyof Route['response']] | InlineHandlerResponse<Route['response'] & MacroContext['response']>>);
...
export type LocalHook<Input extends BaseMacro, Schema extends RouteSchemaWithResolvedMacro, Singleton extends SingletonBase, Errors extends {
    [key in string]: Error;
}, Parser extends keyof any = ''> = (Input extends any ? Input : Prettify<Input>) & {
    detail?: DocumentDecoration;
    /**
     * Short for 'Content-Type'
     *
...
export interface MacroProperty<in out TypedRoute extends RouteSchema = {}, in out Singleton extends SingletonBase = {
    decorator: {};
    store: {};
    derive: {};
    resolve: {};
...
export interface Macro<in out Input extends BaseMacro = {}, in out TypedRoute extends RouteSchema = {}, in out Singleton extends SingletonBase = {
    decorator: {};
    store: {};
    derive: {};
    resolve: {};
}, in out Errors extends Record<string, Error> = {}> {
    [K: keyof any]: MaybeValueOrVoidFunction<Input & MacroProperty<TypedRoute, Singleton, Errors>>;
}
...
type ReconcileStatus<A extends Record<number, unknown>, B extends Record<number, unknown>> = {
    [K in keyof A | keyof B]: K extends keyof A ? A[K] : B[K];
};
export type ComposeElysiaResponse<in out Schema extends RouteSchema, in out Handle, in out Possibility extends PossibleResponse> = Prettify<ReconcileStatus<Schema['response'], UnionResponseStatus<ValueOrFunctionToResponseSchema<Handle>, Possibility & (EmptyRouteSchema extends Pick<Schema, keyof EmptyRouteSchema> ? {} : {
    422: {
        type: 'validation';
        on: string;
        summary?: string;
...
export type ExcludeElysiaResponse<T> = PartialIf<Exclude<Awaited<T>, AnyElysiaCustomStatusResponse> extends infer A ? IsNever<A & {}> extends true ? {} : // Intersect all union and fallback never to {}
A & {} : {}, undefined extends Awaited<T> ? true : false>;
/**
 * @deprecated
 */
 ...
 * @deprecated
 */
export type InferHandler<T extends AnyElysia, Path extends string = T['~Prefix'], Schema extends RouteSchema = T['~Metadata']['schema']> = InlineHandler<MergeSchema<Schema, T['~Metadata']['schema'], Path>, T['~Singleton'] & {
    derive: T['~Ephemeral']['derive'] & T['~Volatile']['derive'];
    resolve: T['~Ephemeral']['resolve'] & T['~Volatile']['resolve'];
}>;
...
type SetContentType = 'application/octet-stream' | 'application/vnd.ms-fontobject' | 'application/epub+zip' | 'application/gzip' | 'application/json' | 'application/ld+json' | 'application/ogg' | 'application/pdf' | 'application/rtf' | 'application/wasm' | 'application/xhtml+xml' | 'application/xml' | 'application/zip' | 'text/css' | 'text/csv' | 'text/calendar' | 'text/event-stream' | 'text/html' | 'text/javascript' | 'text/plain' | 'text/xml' | 'image/avif' | 'image/bmp' | 'image/gif' | 'image/x-icon' | 'image/jpeg' | 'image/png' | 'image/svg+xml' | 'image/tiff' | 'image/webp' | 'multipart/mixed' | 'multipart/alternative' | 'multipart/form-data' | 'audio/aac' | 'audio/x-midi' | 'audio/mpeg' | 'audio/ogg' | 'audio/opus' | 'audio/webm' | 'video/x-msvideo' | 'video/quicktime' | 'video/x-ms-wmv' | 'video/x-msvideo' | 'video/x-flv' | 'video/av1' | 'video/mp4' | 'video/mpeg' | 'video/ogg' | 'video/mp2t' | 'video/webm' | 'video/3gpp' | 'video/3gpp2' | 'font/otf' | 'font/ttf' | 'font/woff' | 'font/woff2' | 'model/gltf+json' | 'model/gltf-binary';
...
export type SSEPayload<Data extends unknown = unknown, Event extends string | undefined = string | undefined> = {
    /** id of the event */
...
export type UnionResponseStatus<A, B> = {} extends A ? B : {} extends B ? A : {
    [key in keyof A | keyof B]: key extends keyof A ? key extends keyof B ? A[key] | B[key] : A[key] : key extends keyof B ? B[key] : never;
};
export type CreateEdenResponse<Path extends string, Schema extends RouteSchema, MacroContext extends RouteSchema, Res extends PossibleResponse> = RouteSchema extends MacroContext ? {
    body: Schema['body'];
    ...
    params: IsNever<keyof (Schema['params'] & MacroContext['params'])> extends true ? ResolvePath<Path> : Prettify<Schema['params'] & MacroContext['params']>;
